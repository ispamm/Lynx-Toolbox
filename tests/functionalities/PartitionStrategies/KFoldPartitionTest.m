classdef KFoldPartitionTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testInvalidKFoldPartition(testCase)
           testCase.assertError(@() KFoldPartition(0.3), 'Lynx:Validation:InvalidInput'); 
           testCase.assertError(@() KFoldPartition(0), 'Lynx:Validation:InvalidInput'); 
        end
        ...
        function testInvalidFoldNumber(testCase)
            p = KFoldPartition(3);
            p = p.partition(1:8);
            testCase.assertError(@() p.setCurrentFold(4), 'Lynx:Validation:InvalidInput');
        end
