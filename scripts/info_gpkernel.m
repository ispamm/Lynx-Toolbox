% INFO_GPKERNEL Print information about GP-based kernel search
...
assert(length(s.datasets) == 1, 'Can only print information for a run with a single dataset');

% Search the GP-based algorithms
gpalgoIndexes = find_algorithms('Kernelsearch_GP', s.algorithms);

nFound = length(gpalgoIndexes);
assert(nFound > 0, 'No Kernelsearch_GP algorithms found');

% Get averaged statistics
stats = cell(nFound, s.nRuns);
for i=1:nFound
    currentAlgo = squeeze(s.trainedAlgo(1, gpalgoIndexes(i), :));
    for j = 1:s.nRuns
...
if(PRINT_KERNELS)
    ...
    for i=1:nFound
        fprintf('\t Algorithm: %s\n', s.algorithms.get(gpalgoIndexes(i)).name);
...
if(PRINT_GRAPHS)
    ...
    for i = 1:nFound
        algoNames{i} = s.algorithms.get(gpalgoIndexes(i)).name;
    end
