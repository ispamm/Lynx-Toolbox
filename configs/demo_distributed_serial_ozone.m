% DEMO_DISTRIBUTED_SERIAL_OZONE - Data-distributed RVFL (serial, ozone)

add_model('B', 'Baseline', @BaselineModel);
add_model('ELM', 'Centralized ELM', @ExtremeLearningMachine, 'hiddenNodes', 350);
add_model('LOC-ELM', 'Local ELM', @ExtremeLearningMachine, 'hiddenNodes', 350);
add_model('CONS-ELM', 'Distributed ELM (Consensus)', @ExtremeLearningMachine, 'hiddenNodes', 350);
add_model('ADMM-ELM', 'ADMM-ELM', @ExtremeLearningMachine, 'hiddenNodes', 350);

set_training_algorithm('ELM', @RegularizedELM, 'C', 300);
set_training_algorithm('LOC-ELM', @SerialDataDistributedRVFL, 'C', 1/300, 'consensus_max_steps', 0);
set_training_algorithm('CONS-ELM', @SerialDataDistributedRVFL, 'C', 1/300, 'consensus_max_steps', 250, 'consensus_thres', 0.001);
set_training_algorithm('ADMM-ELM', @SerialDataDistributedRVFL, 'C', 1/300, 'train_algo', 'admm', ...
...
add_feature(InitializeTopology(RandomTopology(20, 0.2), 'disable_parallel', 'distribute_data', 'disable_plot'));
