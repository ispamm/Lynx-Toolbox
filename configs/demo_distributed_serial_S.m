% DEMO_DISTRIBUTED_SERIAL_S - Data-distributed RVFL (serial, Sylva)

%add_model('B', 'Baseline', @BaselineModel);
%add_model('ELM', 'Centralized ELM', @ExtremeLearningMachine, 'hiddenNodes', 450);
%add_model('LOC-ELM', 'Local ELM', @ExtremeLearningMachine, 'hiddenNodes', 450);
%add_model('CONS-ELM', 'Distributed ELM (Consensus)', @ExtremeLearningMachine, 'hiddenNodes', 450);
add_model('ADMM-ELM', 'ADMM-ELM', @ExtremeLearningMachine, 'hiddenNodes', 450);

%set_training_algorithm('ELM', @RegularizedELM, 'C', 40);
%set_training_algorithm('LOC-ELM', @SerialDataDistributedRVFL, 'C', 1/40, 'consensus_max_steps', 0);
%set_training_algorithm('CONS-ELM', @SerialDataDistributedRVFL, 'C', 1/40, 'consensus_max_steps', 250, 'consensus_thres', 0.01);
set_training_algorithm('ADMM-ELM', @SerialDataDistributedRVFL, 'C', 1/40, 'train_algo', 'admm', ...
    'consensus_max_steps', 250, 'admm_max_steps', 250, 'admm_rho', 1, 'admm_reltol', 10^0, 'admm_abstol', 0.001);
...
add_feature(InitializeTopology(RandomTopology(30, 0.2), 'disable_parallel', 'distribute_data', 'disable_plot'));
