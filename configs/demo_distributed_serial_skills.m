% DEMO_DISTRIBUTED_SERIAL_SKILLS - Data-distributed RVFL (serial, skills)

add_model('B', 'Baseline', @BaselineModel);
add_model('ELM', 'Centralized ELM', @ExtremeLearningMachine, 'hiddenNodes', 400);
add_model('LOC-ELM', 'Local ELM', @ExtremeLearningMachine, 'hiddenNodes', 400);
add_model('CONS-ELM', 'Distributed ELM (Consensus)', @ExtremeLearningMachine, 'hiddenNodes', 400);
add_model('ADMM-ELM', 'ADMM-ELM', @ExtremeLearningMachine, 'hiddenNodes', 400);

set_training_algorithm('ELM', @RegularizedELM, 'C', 4);
set_training_algorithm('LOC-ELM', @SerialDataDistributedRVFL, 'C', 1/4, 'consensus_max_steps', 0);
set_training_algorithm('CONS-ELM', @SerialDataDistributedRVFL, 'C', 1/4, 'consensus_max_steps', 250, 'consensus_thres', 0.001);
set_training_algorithm('ADMM-ELM', @SerialDataDistributedRVFL, 'C', 1/4, 'train_algo', 'admm', ...
...
add_dataset('S', 'Skills', 'uci_skills');
...
add_feature(InitializeTopology(RandomTopology(10, 0.2), 'disable_parallel', 'distribute_data', 'disable_plot'));
