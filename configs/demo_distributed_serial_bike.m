% DEMO_DISTRIBUTED_SERIAL_BIKE - Data-distributed RVFL (serial, bike)

add_model('B', 'Baseline', @BaselineModel);
add_model('ELM', 'Centralized ELM', @ExtremeLearningMachine, 'hiddenNodes', 800);
add_model('LOC-ELM', 'Local ELM', @ExtremeLearningMachine, 'hiddenNodes', 800);
add_model('CONS-ELM', 'Distributed ELM (Consensus)', @ExtremeLearningMachine, 'hiddenNodes', 800);
add_model('ADMM-ELM', 'ADMM-ELM', @ExtremeLearningMachine, 'hiddenNodes', 800);

set_training_algorithm('ELM', @RegularizedELM, 'C', 1024);
set_training_algorithm('LOC-ELM', @SerialDataDistributedRVFL, 'C', 1/1024, 'consensus_max_steps', 0);
set_training_algorithm('CONS-ELM', @SerialDataDistributedRVFL, 'C', 1/1024, 'consensus_max_steps', 250, 'consensus_thres', 0.01);
set_training_algorithm('ADMM-ELM', @SerialDataDistributedRVFL, 'C', 1/1024, 'train_algo', 'admm', ...
...
add_dataset('B', 'Bike', 'bikesharing');
...
add_performance(Tasks.R, MeanSquaredError());
